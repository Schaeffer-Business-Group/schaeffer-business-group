...
app.use(Express.static(Path.join(__dirname, 'schaeffer-business-group-site')));

app.get('/*', (_request, response) => {
  response.sendFile(
    Path.join(__dirname, 'schaeffer-business-group-site', 'index.html')
  );
});
...
const callbackFn = (p: number) => {
  console.log(`Schaeffer Business Group 🚀 app started on port ${p}`);
};
